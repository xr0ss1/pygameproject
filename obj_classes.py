...
class Player(pygame.sprite.Sprite):
    ...
    def init_guns(self):
        self.guns = []
        self.guns.append([pygame.image.load('sprites/pl_pistol.png').convert_alpha(), 7, 25, 7, 35, 20])
        # 2- shoot timer 3 - ammo count 4 - reload timer 5 - bullet speed
        self.guns.append([pygame.image.load('sprites/pl_auto.png').convert_alpha(), 7, 15, 20, 85, 20])
    ...
    def rotate(self, cam):
        ...
        if (legs_deg < 315) and (legs_deg > 45):
            ...
            if legs_deg == 315:
                if (225 <= angle <= 360) or (45 >= angle >= 0):
                    self.rot_image = pygame.transform.rotate(self.gun_image, angle)
            if legs_deg == 45:
                if (315 <= angle <= 360) or (135 >= angle >= 0):
                    self.rot_image = pygame.transform.rotate(self.gun_image, angle)
            if legs_deg == 0:
    ...
    def draw(self, par, cam):
        ...
        self.gun_rect = self.rot_image.get_rect(
            center=(self.rect.centerx - cam.plpos[0], self.rect.centery - cam.plpos[1]))
        self.legs_rect = self.leg_image.get_rect(
            center=(self.rect.centerx - cam.plpos[0], self.rect.centery - cam.plpos[1]))
        ...
        self.rect.x + int(math.cos(rad) * 30)
        self.rect.y + int(math.sin(rad) * 30)
